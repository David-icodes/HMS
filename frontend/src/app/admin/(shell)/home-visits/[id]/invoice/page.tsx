...
export default function AdminHomeVisitInvoicePage() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await adminFetch<{ data: HomeVisit }>(`/api/admin/home-visits/${params.id}`);
      setHv(res.data);
    } catch {
      /* handled */
    } finally {
  ...
  const generate = async () => {
    ...
    setGenerating(true);
    try {
      const res = await adminFetch<{ data: { homeVisit: HomeVisit } }>(`/api/admin/home-visits/${hv._id}/invoice`, { method: 'POST', body: {} });
  ...
  return (
    <div className="space-y-5">
      <div className="print:hidden">
        <Link href="/admin/home-visits" className="inline-flex items-center gap-1.5 text-sm text-slate-600 hover:text-teal-700">
